...
def extract_patches(slide_path, mask_path, patch_size, patches_output_dir, slide_id, max_patches_per_slide=2000):
    ...
    print(f"Processing slide: {patch_folder}")
        ...
        for x, y in indices:
            ...
            if mask[x_mask, y_mask] == 1:
                ...
                if (mask_patch.sum() > BACKGROUND_THRESHOLD * mask_patch.size) and not (is_low_contrast(patch)):
                    ...
                    # save the patch as a svg image
                    svg_path = os.path.join("svg_patches", f"{x}_{y}.svg")
                    Image.fromarray(patch.convert('RGB')).save(svg_path)
                    tile_name = f"{x}_{y}"
...
def get_slide_id(slide_name):
    return slide_name.split('/')[-1].split('.')[0]
...
parser.add_argument('--gpu_id', default=3, type=int,
                    help='GPU device ID to use (0, 1, 2, etc.). If not specified, uses CPU.')
...
if __name__ == '__main__':
    ...
    if args.ref_file:
        ...
        wsi_files = [s for s in selected_slides]
        slide_list = list(set(wsi_files))
    ...
    # list the svs files in the folder or subfolders
    # list folders in /srv2/yson2999/bulk_rna
    # folders = [os.path.join(root, file) 
    #            for root, dirs, files in os.walk('/srv2/yson2999/temp_folder')
    #            for file in files
    #            if file.endswith('.svs')]
    # print(len(folders))
    # svs_files = [os.path.join(root, dirs) 
    #              for root, dirs, files in os.walk('/srv2/yson2999/bulk_rna')
    #              for file in files
    #              if file.endswith('.svs')]
    # print(svs_files)

    opts = [
        (s, (args.patch_size, args.patch_size), args.patch_path, args.mask_path,
        get_slide_id(s), args.max_patches_per_slide) for
        (i, s) in enumerate(slide_list)]
    # # opts = [('/srv2/yson2999/bulk_rna/TCGA_slides/TCGA_LUAD/18a048fd-ece4-46ed-beb6-da5243466d34/TCGA-55-8207-01Z-00-DX1.2dafc442-f927-4b0d-b197-cc8c5f86d0fc.svs',
    # # (args.patch_size, args.patch_size), args.patch_path, args.mask_path,
    # #     get_slide_id('/srv2/yson2999/bulk_rna/TCGA_slides/TCGA_LUAD/18a048fd-ece4-46ed-beb6-da5243466d34/TCGA-55-8207-01Z-00-DX1.2dafc442-f927-4b0d-b197-cc8c5f86d0fc.svs'), args.max_patches_per_slide)]
