...
if __name__ == '__main__':
    ...
    if args.feat_type == 'resnet':
        ...
        local_dir = "/srv2/yson2999/bulk_rna/weights" # add dir for saved model
        ...
        model.load_state_dict(torch.load(os.path.join(local_dir, 
                                    "pytorch_model_uni.bin"), map_location="cpu"), strict=True)
        model.to(device)
        model.eval()
    ...
    # load txt file to get the slide_list
    # with open('/srv2/yson2999/bulk_rna/BRCA/uni_features/failed_features_slides_brca.txt', 'r') as f:
    #     failed_slides = f.readlines()
    # failed_slides = [s.strip() for s in failed_slides]
    # failed_slides = []

    for i, row in tqdm(df.iterrows()):
        WSI = row['wsi_file_name'].split('/')[-1]
        ...
        try:
            ...
            print(e)
            print(WSI)
            # failed_slides.append(WSI)
            # # save failed slides to txt file
            # with open(os.path.join(args.feature_path, "failed_features_slides_resnet.txt"), 'w') as f:
