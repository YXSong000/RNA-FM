...
class ViT(nn.Module):
    def __init__(self, *, num_outputs=20820, dim=1024, depth=6, heads=16, mlp_dim=2048, dim_head = 64,
...
def train(model, dataloaders, optimizer, accelerator=None,
    ...
    for epoch in tqdm(range(num_epochs)):
        for phase in phases:
            ...
            for s, (image, rna_data, _) in enumerate(dataloaders[phase]):
                if image == []: continue
...
def evaluate(model, dataloader, run=None, verbose=True, suff='', cohort=None):
    model.eval()
    loss_fn = nn.MSELoss()
    losses = []
    preds = []
    ...
    maes = []
    smapes = []
    for image, rna_data, wsi_file_name in tqdm(dataloader):
        ...
        projs.append([cohort * image.shape[0]])
