...
def pathway_smoothness(pred, pathway_indices):
    loss = 0.
    for idx in pathway_indices:
        g = pred[:, idx]                      # [B,K]
        g = g - g.mean(dim=1, keepdim=True)
        loss += g.var(dim=1).mean()
    return loss / len(pathway_indices)
...
def main(args):
    ...
    for fold in range(args.k):
        wandb.init(project='Diffusion_rna', config=args, name=args.exp_name+'_fold_'+str(fold)) 
        logger.info(f'Fold {fold} is starting ...')
        test_df = df
        # init dataset
        test_dataset = SuperTileRNADataset(test_df, args.feature_path)

        args.num_genes = test_dataset.num_genes 
        args.feature_dim = test_dataset.feature_dim
        ...
        os.makedirs(os.path.join(save_dir, f'split_{fold}'), exist_ok=True)
        save_path = os.path.join(save_dir, f'split_{fold}')
        
        ckpts = sorted(glob(os.path.join(args.ckpt_path, f'split_{fold}', '**.pt')))
        ckpt = ckpts[-1]
        ...
        preds, preds_ema, real, wsis, projs = evaluate(model, test_dataloader, ema, logger=logger, sample_fn=sample_fn, split=fold, device=device, args=args)
        ...
        with open(os.path.join(save_path, f'test_results_{fold}_cfg_{args.cfg_scale}.pkl'), 'wb') as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ckpt_path', type=str, default='./rna_ckpts/output/LUAD/RNAFM_LUAD', help='path to checkpoint')
    ...
    # Re-parse arguments to include mode-specific arguments
    args = parser.parse_args()

    wandb.login(key="eceea4ac99114c9d7406f9e536598a22e9564ff9")
