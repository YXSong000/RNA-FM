...
from utils import custom_collate_fn, filter_no_features, load_patient_kfold
...
def he2rna_predict(model, dataloader):
    ...
    projs = []
    labels = []
    for x, y, wsi_file_name in dataloader:
        x = x.float().to(model.device)
...
def fit(model,
    ...
    if (test_loader != None):
        preds_test, labels_test, wsis = he2rna_predict(model, test_loader)
        return preds_test, labels_test, wsis
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cohort', type=str, default="BRCA", help='cohort')
    ...
    parser.add_argument('--log', type=str, default="sequoia", help='Whether to do the log with wandb')
    ...
    for train_idx, val_idx in zip(train_idxs, val_idxs):
        ...
        print(f"Number of genes: {train_dataset.num_genes}")
        ...
        preds_random, labels_random, wsis_rand = he2rna_predict(model, valid_loader)

        preds, labels, wsis = fit(model=model,
                                            lr=args.lr,
