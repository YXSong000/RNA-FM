...
def filter_no_features(df, feature_path, feature_name):
    ...
    wsis_with_features = os.listdir(os.path.join(feature_path))
    for wsi in wsis_with_features:
        try:
            with h5py.File(os.path.join(feature_path, wsi, wsi+'.h5'), "r") as f:
                cols = list(f.keys())
                # print(f'Cols: {cols}')
                if feature_name not in cols:
                    remove.append(wsi)
                    print(f'Removing {wsi} because {feature_name} not in cols')
        except Exception as e:
            remove.append(wsi)    
        
    all_wsis_with_features += wsis_with_features
    remove += df[~df['wsi_file_name'].str.split('/').str[-1].str.replace('.svs', '', regex=False).isin(all_wsis_with_features)].wsi_file_name.values.tolist()
    print(f'Remove: {len(remove)}')
    print(f'Original shape: {df.shape}')
    df = df[~df['wsi_file_name'].str.split('/').str[-1].str.replace('.svs', '', regex=False).isin(remove)].reset_index(drop=True)
...
def load_patient_kfold(dataset, path):
    patient_split = np.load(path, allow_pickle=True).item()
    indices = np.arange(len(dataset))
    train_idx = []
    test_idx = []

    for fold in range(len(patient_split)):
        train_idx.append(indices[np.any(np.array(dataset.patient_id)[:, np.newaxis] ==
                                       np.array(patient_split[f'fold_{fold}']['train'])[np.newaxis], axis=1)])
        train_idx.append(indices[np.any(np.array(dataset.patient_id)[:, np.newaxis] ==
                                       np.array(patient_split[f'fold_{fold}']['val'])[np.newaxis], axis=1)])
        test_idx.append(indices[np.any(np.array(dataset.patient_id)[:, np.newaxis] ==
                                       np.array(patient_split[f'fold_{fold}']['test'])[np.newaxis], axis=1)])
    
    return train_idx, test_idx 

def patient_external_test(dataset, n_splits=5, random_state=0, valid_size=0.1):
