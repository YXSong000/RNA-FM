...
def result_generation(cancers, exp_name, save_path_dir, folds, num_sampling_steps):
    ...
    for cancer in cancers:
        print(cancer.upper())
        file_path = f'./rna_ckpts/output/{cancer.upper()}/{exp_name[idx]}/'
        
        save_path = os.path.join(save_path_dir, cancer.upper())
        ...
        for k in range(folds):
            ...
            report_results.to_csv(os.path.join(save_path, f'report_results_{k}.csv'))

            all_res.to_csv(os.path.join(save_path, f'all_genes_{k}.csv'))
            all_res_ema.to_csv(os.path.join(save_path, f'all_genes_ema_{k}.csv'))
        idx += 1
...
if __name__=='__main__':
    ...
    parser.add_argument('--cancers', type=list, default=['CPTAC_COAD', 'CPTAC_BRCA', 'CPTAC_LUAD'])
    parser.add_argument('--model_dir', type=str, default='results')
    parser.add_argument('--folds', type=int, default=5)
    parser.add_argument('--exp_name', type=str, default=['diff_regene_filtered_normalized_new_1500_uniform_pathway_newloss_filtered_refine_nonormloss_nogate_bg_noval_rampema_COAD', 'diff_regene_filtered_normalized_new_1500_uniform_pathway_newloss_filtered_refine_nonormloss_nogate_bg_noval_rampema_BRCA', 'diff_regene_filtered_normalized_new_1500_uniform_pathway_newloss_filtered_refine_nonormloss_nogate_bg_noval_rampema_replicate'])
    parser.add_argument('--num_sampling_steps', type=int, default=20)
    args = parser.parse_args()
    result_generation(args.cancers, args.exp_name, args.model_dir, args.folds, 20)